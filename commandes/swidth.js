...
hango({
  ...
  try {
    ...
    const cap = `*Screenshot taken by 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4`;
...
hango({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4`;
...
hango({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4 will extract the text for you.");
    }
...
hango({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4 will generate it.");
    }
    const prompt = args.join(" ");
