...
async function relaybug(dest, hn, ms, repondre, amount, victims, bug) {
...
async function sendbug(dest, hn, ms, repondre, amount, victims, bug) {
    for (let i = 0; i < victims.length; i++) {
        if (!phoneRegex.test(victims[i])) {
            repondre(`${victims[i]} not a valid phone number`);
            continue;
...
hango(
    ...
    async (dest, hn, commandOptions) => {
        ...
        let menu = `${mono}👾 Welcome to the Elite Menu by 𝗘𝗹𝗶𝗧𝗲𝗰𝗵𝗪𝗶𝘇-𝗩𝟰! 🚀\nHello ${ms.pushName}\n${timewisher(time)}\n\n\n┗❏${mono}`;
        switch (version) {
            case "v1":
                {
                    hn.sendMessage(
...
hango(
    ...
    async (dest, hn, commandOptions) => {
        ...
        await hn.sendMessage(dest, { react: { text: "✅", key: ms.key } });
    }
);
...
hango(
    ...
    async (dest, hn, commandOptions) => {
        ...
        if (arg.length === 1) {
            ...
            try {
                await sendbug(dest, hn, ms, repondre, amount, victims, bug);
            } catch (e) {
...
hango(
    ...
    async (dest, hn, commandOptions) => {
        ...
        if (arg.length === 1) {
            ...
            await repondre(`sending ${amount} bugs to ${victims[0]}`);
            try {
                await relaybug(dest, hn, ms, repondre, amount, victims, bug);
            } catch (e) {
                await repondre("An error occured");
                console.log(`An error occured: ${e}`);
                await react(dest, hn, ms, "⚠️");
