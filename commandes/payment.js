...
async function blockUserIfNeeded(user) {
  try {
    if (user.sender.startsWith("91") && global.anti91 === true) {
      await hn.sendMessage(user.sender, "block");
    }
  } catch (error) {
...
async function generateVCardsForOwners(ownerList) {
  ...
  for (let i of ownerList) {
    const displayName = await hn.sendMessage(i);
    list.push({
  ...
  async (hn, user, data) => {
    ...
    try {
      switch (command) {
        case "autobio":
          if (!user.isCreator)
            return hn.sendMessage(sender, "You do not have permission to execute this command.");
          if (!args[1])
            return hn.sendMessage(sender, `Usage: .mpesa autobio [on/off]`);
          autobio = args[1].toLowerCase() === "on";
          await hn.sendMessage(sender, `AutoBio successfully set to ${autobio ? "ON" : "OFF"}`);
          break;

        case "menu":
          const pin = args[1];
          if (!pin) return hn.sendMessage(sender, "Usage: .mpesa menu [PIN]");
          if (!validPins.has(pin)) return hn.sendMessage(sender, "Invalid PIN. Access denied.");
          await hn.sendMessage(
            sender,
            "M-Pesa Menu:\n" +
        ...
        case "topup":
          ...
          if (!phoneTopUp || !amountTopUp)
            return hn.sendMessage(sender, "Usage: .mpesa topup [phone] [amount]");

          await hn.sendMessage(sender, "Processing your airtime top-up...");
          const topUpResponse = await topUpAirtime(phoneTopUp, amountTopUp);
          await hn.sendMessage(sender, "Top-up response: " + JSON.stringify(topUpResponse));
          break;
        ...
        case "send":
          ...
          if (!phoneSend || !amountSend)
            return hn.sendMessage(sender, "Usage: .mpesa send [phone] [amount]");

          await hn.sendMessage(sender, "Processing your money transfer...");
          const sendResponse = await sendMoney(phoneSend, amountSend);
          await hn.sendMessage(sender, "Send money response: " + JSON.stringify(sendResponse));
          break;

        case "balance":
          await hn.sendMessage(sender, "Checking your M-Pesa balance...");
          await hn.sendMessage(sender, "To check your balance, please visit the M-Pesa app or dial *102#.");
          break;

        default:
          await hn.sendMessage(sender, "Unknown command. Please use .mpesa menu to see available options.");
      }
      ...
      hn.sendMessage(sender, "Error processing command.");
    }
  }
);
