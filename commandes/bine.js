...
hango({
  ...
  try {
    ...
    try {
      ...
      const caption = "> Thank for choosing 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4";
      await hn.sendMessage(ms, { image: imageBuffer, caption: caption }, { quoted: ms });
    } catch (error) {
  ...
  try {
    ...
    if (/image\/.*/.test(contentType)) {
      ...
      await hn.sendMessage(sender, {
        image: { url: fetchUrl },
        caption: "> Thank you for choosing 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4"
      }, { quoted: context.ms });
    } else if (/video\/.*/.test(contentType)) {
      // Send video message
      await hn.sendMessage(sender, {
        video: { url: fetchUrl },
        caption: "> *Thank for choosing 𝓔𝓵𝓲𝓣𝓮𝓬𝓱𝓦𝓲𝔃-𝓥4"
      }, { quoted: context.ms });
