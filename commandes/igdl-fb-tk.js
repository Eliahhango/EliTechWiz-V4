...
hango({nomCom : "instagram" , categorie : "Download"},async (dest , hn , commandeOptions)=>{
  ...
  try {
    ...
    hn.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader powered by * 𝗘𝗹𝗶𝗧𝗲𝗰𝗵𝗪𝗶𝘇-𝗩𝟰*",gifPlayback : false },{quoted : ms}) 
    }
    else {
        hn.sendMessage(dest,{image : {url : igvid.data.data.data[0].url},caption : "ig image downloader powered by * 𝗘𝗹𝗶𝗧𝗲𝗰𝗵𝗪𝗶𝘇-𝗩𝟰*"})
    }
