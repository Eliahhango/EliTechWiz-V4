...
hango({ nomCom: "videologo", categorie: "modern-logo", reaction: "✋" }, async (dest, hn, commandeOptions) => {
...
*Enjoy 😂*`;
    ...
    // Send the message
    const sentMessage = await hn.sendMessage(dest, messageToSend, { quoted: ms });
      ...
      if (message.message.extendedTextMessage.contextInfo && message.message.extendedTextMessage.contextInfo.stanzaId === sentMessage.key.id) {
        ...
        if (logoUrl) {
          await hn.sendMessage(dest, {
            video: { url: logoUrl },
            mimetype: "video/mp4",
            caption: `*Downloaded by 𝗘𝗹𝗶𝗧𝗲𝗰𝗵𝗪𝗶𝘇-𝗩𝟰*`,
          }, { quoted: ms });
        }
      }
    });
