const { hango } = require("../eliah/hango");

hango({ nomCom: "vv", categorie: "General", reaction: "😅" }, async (dest, hn, commandeOptions) => {
    ...
    if (viewOnceMsg) {
        try {
            ...
            if (viewOnceMsg.imageMessage) {
                const imagePath = await hn.downloadAndSaveMediaMessage(viewOnceMsg.imageMessage); // Download image
            ...
            // Handle view-once video messages
                ...
                const videoPath = await hn.downloadAndSaveMediaMessage(viewOnceMsg.videoMessage); // Download video
            ...
            // Handle view-once audio messages
                ...
                const audioPath = await hn.downloadAndSaveMediaMessage(viewOnceMsg.audioMessage); // Download audio
                await hn.sendMessage(dest, {
                    audio: { url: audioPath },
