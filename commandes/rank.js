...
const {getMessagesAndXPByJID,getBottom10Users} = require("../ess/level");
...
function get_level_exp(xp) {
    const levelThresholds = [
        ...
        { level: 'hn-GOD', xpThreshold: 2000000}
    ];
  ...
  if (msgRepondu) {
       ...
       try {
        ...
        const data = await get_level_exp(rank.xp)
         ...
         try {
              ppuser = await hn.profilePictureUrl(auteurMsgRepondu , 'image') ;
         } catch {
            ppuser = mybotpic()
         } ;
     ...
     hn.sendMessage( 
      ...
      try {
        ...
        const data =  get_level_exp(rang.xp)
         ...
         try {
              ppuser = await hn.profilePictureUrl(jid, 'image') ;
         } catch {
            ppuser = mybotpic()
         } ;
