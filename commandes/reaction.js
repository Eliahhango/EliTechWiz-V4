...
const generateReactionCommand = (reactionName, reactionEmoji) => {
    ...
    async (origineMessage, hn, commandeOptions) => {
        ...
        try {
            ...
            // Envoyer la vidéo avec hango
            if (msgRepondu) { 
              var txt =` @${auteurMessage.split("@")[0]}  ${reactionName} @${auteurMsgRepondu.split("@")[0]}`
       hn.sendMessage(origineMessage, { video: videoBuffer,gifPlayback: true,caption:txt,mentions:[auteurMessage,auteurMsgRepondu] }, { quoted: ms });
                ...
                hn.sendMessage(origineMessage, videoMessage, { quoted: ms });
            }
