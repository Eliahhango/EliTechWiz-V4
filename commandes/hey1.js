...
hango({ nomCom: "tourl7", categorie: "General", reaction: "💗" }, async (origineMessage, hn, commandeOptions) => {
    ...
    if (msgRepondu.videoMessage) {
        ...
        mediaPath = await hn.downloadAndSaveMediaMessage(msgRepondu.audioMessage);
        mediaType = 'audio';
    ...
    try {
        ...
        switch (mediaType) {
            case 'image':
                repondre(`Eliah tech url: ${catboxUrl}`);
                break;
            case 'video':
                repondre(`Eliah tech url: ${catboxUrl}`);
                break;
